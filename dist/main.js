...
/***/ function(module, exports, __webpack_require__) {
	...
	var _utilQueryStringJs = __webpack_require__(10);

	var _utilQueryStringJs2 = _interopRequireDefault(_utilQueryStringJs);

	var _ajaxAjaxJs = __webpack_require__(11);
	...
	qshObject = Object.assign(qshObject, device);
	...
	qshObject.getQueryStringByName = _utilQueryStringJs2['default'];
	qshObject.compileTpl = _utilCompileTplJs2['default'];

	window.qshObject = qshObject;
	    ...
	    if (device.shell === 'wechat') {
	        ...
	        if (openid) {
	            ...
	            if (!code) {
	                ...
	                window.weixin_code_promise = +(function () {
...
/***/ function(module, exports) {
	...
	var shell = ua.match(/MicroMessenger/i) ? 'wechat' : 'common';
...
/***/ function(module, exports, __webpack_require__) {
	...
	function toast() {
	    var type = arguments.length <= 0 || arguments[0] === undefined ? 'success' : arguments[0];
	    var content = arguments.length <= 1 || arguments[1] === undefined ? type === 'loading' ? '正在加载中...' : '已完成' : arguments[1];
	    return (function () {
	        var toast_mount = toast_mount_map[type];

	        if (toast_mount) {
	            toast_mount.querySelector('.weui_toast_content').textContent = content;
	            return autoHideToast(type);
	        } else {
	            var tpl = type === 'loading' ? loading_tpl : common_tpl;
	            var docfrag = document.createDocumentFragment();
	            var tmp = docfrag.appendChild(document.createElement('div'));
	            tmp.innerHTML = (0, _utilCompileTplJs2['default'])(tpl, {
	                content: content
	            });

	            toast_mount_map[type] = document.body.appendChild(tmp.lastChild);
	            return autoHideToast(type);
	        }
	    })();
	}
	...
	function autoHideToast(type) {
	    toast_mount_map[type].style.display = 'block';
	    if (type === 'loading') {
	        return {
	            hide: function hide() {
	                toast_mount_map[type].style.display = 'none';
	            }
	        };
	    } else {
	        clearTimeout(timer);
...
/***/ function(module, exports) {
	...
	var xConfig = {value:123};

	'use strict';

	console.log(xConfig.value);
